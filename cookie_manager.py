...
class CookieManager:
    ...
    def __init__(self, cookie_file: str = "cookie.txt"):
        ...
        # 网易云音乐登录态 Cookie。网页版通常只有 MUSIC_U，部分客户端会提供 MUSIC_A。
    ...
    def is_cookie_valid(self) -> bool:
        ...
        try:
            ...
            # 检查是否存在至少一种登录态 Cookie。不同登录入口返回的字段不同，
            # Web 版常见 MUSIC_U，部分客户端可能是 MUSIC_A。
            cookie_names = set(cookies.keys())
            present_auth_cookies = self.important_cookies & cookie_names
            if not present_auth_cookies:
                self.logger.warning("缺少登录Cookie: MUSIC_U/MUSIC_A")
                return False

            # 检查登录态 Cookie 是否有效（基本验证）
            auth_cookie = cookies.get('MUSIC_U') or cookies.get('MUSIC_A', '')
            if not auth_cookie or len(auth_cookie) < 10:
                self.logger.warning("登录Cookie无效")
                return False
    ...
    def get_cookie_info(self) -> Dict[str, Any]:
        ...
        try:
            cookies = self.parse_cookies()
            cookie_names = set(cookies.keys())
            missing_auth_cookies = [] if self.important_cookies & cookie_names else list(self.important_cookies)
            ...
            info = {
                ...
                'important_cookies_present': list(self.important_cookies & cookie_names),
                'missing_important_cookies': missing_auth_cookies,
