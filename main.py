...
def download_playlist_api():
    ...
    try:
        ...
        fd, zip_filename = tempfile.mkstemp(
            ...
            suffix=".zip",
            dir=str(api_service.downloads_path)
        )
        os.close(fd)
        ...
        with zipfile.ZipFile(zip_path, 'w', compression=zipfile.ZIP_DEFLATED) as zip_file:
            for index, track in enumerate(tracks, start=1):
                ...
                try:
                    ...
                    if not download_result.success:
                        raise DownloadException(download_result.error_message or "未知下载错误")

                    source_path = Path(download_result.file_path or "")
                    if not source_path.exists():
                        raise DownloadException("下载完成但文件不存在")

                    arc_base = api_service.downloader._sanitize_filename(
            ...
            if failures:
                ...
                zip_file.writestr("下载失败列表.txt", failure_text)
        ...
        response.headers['X-Download-Message'] = 'Playlist download completed'
        response.headers['X-Download-Filename'] = quote(download_name, safe='')

        def cleanup_zip_file(path: Path = zip_path) -> None:
            try:
                if os.path.exists(path):
                    os.remove(path)
            except Exception as cleanup_error:
                api_service.logger.warning(f"临时ZIP清理失败: {cleanup_error}")

        response.call_on_close(cleanup_zip_file)
        return response
...
def api_info():
    ...
    try:
        info = {
            ...
            'supported_qualities': VALID_QUALITIES,
            'config': {
                'downloads_dir': str(api_service.downloads_path.absolute()),
